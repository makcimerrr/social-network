...
func ComingEventHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		var id int
		var comeStatus bool
		err = db.QueryRow("SELECT ID, Come FROM coming_event WHERE event_id = ? AND user_id = ?", come.Event_id, come.User_id).Scan(&id, &comeStatus)
		...
		if err == nil {
			...
			_, err = db.Exec("INSERT INTO coming_event (user_id, event_id, come) VALUES (?, ?, true)", come.User_id, come.Event_id)
			_, err = db.Exec("UPDATE EVENTGROUPS SET Come = Come + 1 WHERE IDEvent = ?", come.Event_id)
		} else {
